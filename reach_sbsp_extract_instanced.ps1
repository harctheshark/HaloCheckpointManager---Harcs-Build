# Offline extractor: reads a Reach HREK source .scenario_structure_bsp via ManagedBlam, walks each instanced-geometry
# definition's winged-edge collision_bsp, transforms it by every instance's placement, and writes a world-space
# triangle blob that HCM loads into the "Instanced Geometry" viewer. No HREK needed by end users (blob ships).
# Usage: pwsh -File reach_sbsp_extract_instanced.ps1 <tag-relative-path-no-ext> <out.rinst>
param(
  [string]$TagRel = "levels\solo\m50\m50_000",
  [string]$Out    = "G:\SteamLibrary\steamapps\common\HREK\m50_000.rinst"
)
...
function VPoint($el){ return $el.Fields[0].Data }   # .Fields is a TagField[] array; field[0] = the point (3 floats)

# ---- read all definitions -> local collision triangles (winged-edge walk) ----
...
$nDef=ECount $defs
$defTris=New-Object 'System.Collections.Generic.List[object][]' 0
$defTris=@{}   # defIndex -> array of [v0x,v0y,v0z,v1..,v2..] (9 floats), local space
$defWithColl=0
for($di=0;$di -lt $nDef;$di++){ $d=EItem $defs $di
  ...
  # cache edges: [startV,endV,fwdE,revE,leftS,rightS]
  $E=New-Object 'int[][]' $ne
  ...
  if($tris.Count -gt 0){ $defTris[$di]=$tris.ToArray(); $defWithColl++ }
}
Write-Host "definitions=$nDef withCollision=$defWithColl"

# ---- read instances, transform def tris -> world, write blob ----
...
$bw.Write([byte[]][char[]]'RINST001')   # magic
$countPos=$fs.Position; $bw.Write([int]0)   # placeholder tri count
$total=0
...
Write-Host "instances=$nInst  world triangles written=$total  -> $Out  ($([int]((Get-Item $Out).Length/1KB)) KB)"
Write-Host "DONE"
