# Quick extraction of m50's 5 zone-set BSPs -> deploy directly to BOTH DLL trees' reach_instanced\ (mirrored paths),
# so the zone-set-aware loader can be tested on m50 without waiting for the full 168-BSP batch.
...
function ExtractBsp($tagRel, $outPath){
  ...
  $nDef=ECount $defs; $defTris=@{}
  for($di=0;$di -lt $nDef;$di++){ $d=EItem $defs $di
    $base="Struct:collision info[0]"; $sb=$null; try{ $sb=$d.SelectField("$base/Block:surfaces") }catch{}
    if($sb -eq $null){ continue }; $ns=ECount $sb; if($ns -le 0){ continue }
    $eb=$d.SelectField("$base/Block:edges"); $vb=$d.SelectField("$base/Block:vertices"); $ne=ECount $eb; $nv=ECount $vb
  ...
  $bw.Write([byte[]][char[]]'RINST001'); $cp=$fs.Position; $bw.Write([int]0); $total=0
  for($ii=0;$ii -lt $nInst;$ii++){ $e=EItem $inst $ii
    ...
    $fx=$fw[0]*$sc;$fy=$fw[1]*$sc;$fz=$fw[2]*$sc;$lx=$lf[0]*$sc;$ly=$lf[1]*$sc;$lz=$lf[2]*$sc;$ux=$up[0]*$sc;$uy=$up[1]*$sc;$uz=$up[2]*$sc
    $px=$po[0];$py=$po[1];$pz=$po[2]
    foreach($t in $defTris[$mi]){ for($v=0;$v -lt 9;$v+=3){ $vx=$t[$v];$vy=$t[$v+1];$vz=$t[$v+2]
      $bw.Write([single]($px+$fx*$vx+$lx*$vy+$ux*$vz)); $bw.Write([single]($py+$fy*$vx+$ly*$vy+$uy*$vz)); $bw.Write([single]($pz+$fz*$vx+$lz*$vy+$uz*$vz)) }; $total++ } }
  $bw.Flush(); $fs.Position=$cp; $bw.Write([int]$total); $bw.Flush(); $bw.Close(); return $total
  } finally { try { $tTF.GetMethod('Dispose',[Type[]]@()).Invoke($tag,@()) } catch {} }
}
$trees=@(
  "C:\Users\hurri\source\repos\HaloCheckpointManager\HCMInternal\HCMExternal\bin\x64\Release\net7.0-windows\reach_instanced",
  "C:\Users\hurri\source\repos\HaloCheckpointManager\HCMExternal\bin\x64\Release\net7.0-windows\reach_instanced")
$bsps=@("levels\solo\m50\m50_000","levels\solo\m50\m50_010","levels\solo\m50\m50_020","levels\solo\m50\m50_001","levels\solo\m50\m50_003")
foreach($rel in $bsps){
  $t=ExtractBsp $rel (Join-Path $trees[0] ($rel+'.rinst'))
  $src=Join-Path $trees[0] ($rel+'.rinst')
  $dst=Join-Path $trees[1] ($rel+'.rinst'); New-Item -ItemType Directory -Force (Split-Path $dst) | Out-Null; Copy-Item $src $dst -Force
  Write-Host ("{0}  tris={1} ({2} KB) -> deployed to both trees" -f $rel,$t,[int]((Get-Item $src).Length/1KB))
}
Write-Host "m50 zone-set blobs deployed."
