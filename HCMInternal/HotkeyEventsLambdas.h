...
public:
...

	{
		...
		constexpr size_t hceToggleCount = 40; // [0..39]  flip a bool
		constexpr size_t hceRadioCount = 4;  // [40..43] set a radio option
		constexpr size_t hceAliasCount = 4;  // [44..47] fire an event that already has its own subscriber
		...
		// [0..39] plain toggles: flip the bool. Same order as the first block of HCE_HOTKEYS.
		const std::array<std::shared_ptr<BinarySetting<bool>>, hceToggleCount> hceToggleTargets
		{
			...
			settings->hceTriggerOverlayShowSector,
		...
		// [40..43] radio options: {the one to switch ON, the sibling to switch OFF}.
		...
		// [44..47] are aliases of events that already exist and already have a subscriber (see the end of the
