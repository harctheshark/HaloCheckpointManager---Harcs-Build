	...
	bool dropRender3DSubscription(bool mustSucceed)
	{
		...
		for (;;)
		{
			...
			if (!mustSucceed)
			{
				...
				PLOG_ERROR << "HCESoftCeilingOverlay: timed out draining the 3D render path; keeping the subscription "
					"rather than freeing state it may still be using";
				return false;
			}

			PLOG_ERROR << "HCESoftCeilingOverlay: still waiting for the 3D render path to finish before teardown; "
